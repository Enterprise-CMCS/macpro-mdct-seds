...
if [[ ! $local_branch =~ $valid_branch ]] || [[ $local_branch =~ $glob ]] || [[ ${#local_branch} -gt 20 ]]; then
    ...
    From Serverless (the original reason for this validation):
        ...
        For Github Actions support, please push your code to a new branch with a name that meets name requirements.
