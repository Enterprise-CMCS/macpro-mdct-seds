...
type WriteLocalApiProxyConfig =
  typeof import("./write-ui-env-file.ts").writeLocalApiProxyConfig;
...
const writeLocalApiProxyConfigMock = mock.fn<WriteLocalApiProxyConfig>(
  async () => undefined
);
...
mock.module("./write-ui-env-file.ts", {
  namedExports: {
    ...
    writeLocalApiProxyConfig: writeLocalApiProxyConfigMock,
  },
});

const bootstrapLocalCognitoUsersMock = mock.fn(async () => undefined);
mock.module("./seedData.ts", {
  namedExports: {
...
describe("runFrontendLocally", () => {
  beforeEach(() => {
    ...
    writeLocalApiProxyConfigMock.mock.resetCalls();
  ...
  it("writes the MiniStack Cognito UI env from stack outputs", async () => {
    ...
    assert.deepEqual(bootstrapLocalCognitoUsersMock.mock.calls[0]?.arguments, [
      "local-user-pool",
    ]);
    assert.deepEqual(
    ...
    assert.deepEqual(writeLocalUiEnvFileMock.mock.calls[0]?.arguments[0], {
      ...
      API_URL: "http://localhost:3333/local-api",
    ...
    assert.deepEqual(writeLocalApiProxyConfigMock.mock.calls[0]?.arguments[0], {
      gatewayEndpoint: "http://127.0.0.1:4566",
      apiBasePath: "/restapis/api123/ministack/_user_request_",
    });
    assert.deepEqual(runCommandMock.mock.calls[0]?.arguments, [
      "ui",
      ["yarn", "start", "--strictPort", "--port", "3333"],
      "services/ui-src",
    ]);
  });
});
