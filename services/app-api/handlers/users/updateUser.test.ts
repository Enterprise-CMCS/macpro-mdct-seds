...
import { scanForUserWithSub as actualScanForUserWithSub } from "./getCurrentUser.ts";
import {
...
describe("updateUser", () => {
  ...
  it("should return an error if the requesting user is updating anything other than their state", async () => {
    ...
    const expectFieldChangeToError = async (changedProperty: any) => {
      const evt = {
  ...
  it("should reject invalid user payloads", async () => {
    ...
    const expectFieldChangeToError = async (payload: any) => {
