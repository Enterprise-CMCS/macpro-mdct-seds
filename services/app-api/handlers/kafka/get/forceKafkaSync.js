...
const batchWrite = async (tableName, items) => {
    console.log(`Performing batchwrite for ${items.length} items in table: ${tableName}`);
    // split items into chunks of 25
    const itemChunks = chunk(items, 25);
    console.log(`Items split into ${itemChunks.length} chunk(s) of not more than 25 items`);
    for (const index in itemChunks) {
        ...
        if ((FailedItems?.length ?? 0) > 0) {
            ...
            console.log(`BatchWrite ran with ${FailedItems.length ?? 0} numbers of failed item updates`);
            console.log(`The following items failed updating for the table ${tableName} -  keys ${keys}`);
        }
    }
};
