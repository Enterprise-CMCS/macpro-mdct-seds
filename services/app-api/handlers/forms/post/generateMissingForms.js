...
export const main = handler(async (event, context) => {
  ...
  const batchWriteMissingForms = async (tryRetryBatch) => {
  ...
  const specifiedYear = data.year;
  const specifiedQuarter = data.quarter.value;

  // Pull list of questions to build new forms
  ...
  // If states not found, return failure message
  if (!allStates.length) {
    return {
      status: 500,
  ...
  //This queries for all form asnwers
  ...
  const misisngForms = determineMissingForms(allStates, currentForms);
  ...
  // LOOP THROUGH JUST MISSING FORMS
  ...
  //     // Process the batch of missing forms
  ...
  //     // Process this batch
  //     await batchWriteMissingForms({ batch: batchRequest, noOfRetries: 0 });
  //   }
...
// QUESTIONS:
// This is both a post and a get,what should it be in serverless.yml
// What are the dates for the quarters ??
// Is it OK to use static state forms JSON or do i need to query the states table?
