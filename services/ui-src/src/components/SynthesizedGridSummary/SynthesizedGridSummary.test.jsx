...
describe("Test SynthesizedGridSummary.js", () => {
  it("should render sub-components", () => {
    ...
    expect(label).toHaveTextContent("5. What is the average number of months of enrollment for of all ages ever enrolled during the quarter?");

    const grid = screen.getByTestId("grid-with-totals");
    expect(grid).toBeInTheDocument();

    const disclaimer = screen.getByTestId("synthesized-disclaimer");
    expect(disclaimer).toHaveTextContent("Values will not appear until source data is provided");
  });
